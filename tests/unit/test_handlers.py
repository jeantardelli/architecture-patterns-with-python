...
class TestAllocate:
    def test_allocates(self):
        bus = bootstrap_test_app()
        bus.handle(commands.CreateBatch("batch01", "COMPLICATED-LAMP", 100, None))
    ...
    def test_commits(self):
        bus = bootstrap_test_app()
        bus.handle(commands.CreateBatch("batch01", "OMINOUS-MIRROR", 100, None))
...
class TestChangeBatchQuantity:
    def test_changes_available_quantity(self):
        bus = bootstrap_test_app()
        bus.handle(commands.CreateBatch("batch01", "ADORABLE-SETTEE", 100, None))
        [batch] = bus.uow.products.get(sku="ADORABLE-SETTEE").batches
        assert batch.available_quantity == 100

        bus.handle(commands.ChangeBatchQuantity("batch01", 50))
        assert batch.available_quantity == 50

    def test_reallocates_if_necessary(self):
        bus = bootstrap_test_app()
        history = [
            commands.CreateBatch("batch01", "INDIFFERENT-TABLE", 50, None),
            commands.CreateBatch("batch02", "INDIFFERENT-TABLE", 50, date.today()),
            commands.Allocate("order01", "INDIFFERENT-TABLE", 20),
            commands.Allocate("order02", "INDIFFERENT-TABLE", 20)]
        for msg in history:
            bus.handle(msg)
        ...
        bus.handle(commands.ChangeBatchQuantity("batch01", 25))
